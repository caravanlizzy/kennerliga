...
class ResultSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if result_config.is_asymmetric:
            if (
                "faction_ids" not in self.initial_data
                or not self.initial_data["faction_ids"]
            ):
                raise serializers.ValidationError(
                    "At least one faction is required for asymmetric games."
                )

        # ✅ Validate starting points system
        if (
            result_config.starting_points_system
            and result_config.starting_points_system.code == "DYNAMIC"
        ):
            if "starting_points" not in data or data["starting_points"] is None:
                raise serializers.ValidationError(
                    "Starting points are required when the system is DYNAMIC."
                )
