...
class ResultConfigSerializer(ModelSerializer):
    ...
    def get_starting_points_system_description(self, obj):
        return (
            obj.starting_points_system.description
    ...
    def update(self, instance, validated_data):
        ...
        if selected_options_data is not None:
            for option_data in selected_options_data:
                ...
                option_instance.choice = option_data.get(
                    "choice", option_instance.choice
                )
...
class FullGameOptionAvailabilityConditionReadSerializer(serializers.ModelSerializer):
    ...
    depends_on_option_name = serializers.CharField(
        source="depends_on_option.name", read_only=True
    )
    expected_choice_name = serializers.CharField(
        source="expected_choice.name", allow_null=True, read_only=True
    )
...
class FullGameOptionAvailabilityGroupReadSerializer(serializers.ModelSerializer):
    conditions = FullGameOptionAvailabilityConditionReadSerializer(
        ...
        for option_payload in options_data:
            ...
            if option_ref:
                if option_ref in option_by_ref:
                    raise serializers.ValidationError(
                        {"options": f"Duplicate option ref: {option_ref}"}
                    )
            ...
            for choice_payload in choices_data:
                ...
                choice = GameOptionChoice.objects.create(
                    option=option, order=ch_order, **choice_payload
                )

                if choice_ref:
                    if choice_ref in choice_by_ref:
                        raise serializers.ValidationError(
                            {"options": f"Duplicate choice ref: {choice_ref}"}
                        )
    ...
    def update(self, instance, validated_data):
        ...
        for option_payload in options_data:
            ...
            if option_id:
                ...
                if option is None:
                    raise serializers.ValidationError(
                        {"options": f"Option id={option_id} unknown."}
                    )
                option.name = option_payload.get("name", option.name)
                option.has_choices = option_payload.get(
                    "has_choices", option.has_choices
                )
                option.order = order
                option.save()
            else:
                option = GameOption.objects.create(
                    game=instance, order=order, **option_payload
                )
            ...
            for choice_payload in choices_data:
                ...
                if choice_id:
                    ...
                    if choice is None:
                        raise serializers.ValidationError(
                            {"options": f"Choice id={choice_id} unknown."}
                        )
        ...
        for opt_id, keep_choice_ids in seen_choice_ids_by_option_id.items():
            GameOptionChoice.objects.filter(option_id=opt_id).exclude(
                id__in=keep_choice_ids
            ).delete()
                ...
                for cond in group_data.get("conditions", []):
                    ...
                    if expected_choice_ref:
                        ...
                        if not expected_choice:
                            raise serializers.ValidationError(
                                {
                                    "options": f"Unknown choice ref: {expected_choice_ref}"
                                }
                            )
...
class BanDecisionSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        league = attrs.get("league") or getattr(self.instance, "league", None)
        selected_game = attrs.get(
            "selected_game"
        )  # populated via source='selected_game'
    ...
    def create(self, validated_data):
        manage_only = (
            self.context.get("request").data.get("manage_only", False)
