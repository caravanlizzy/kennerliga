    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        ...
        if game_id:
            if db_field.name == "option":
                ...
                kwargs["queryset"] = GameOptionChoice.objects.filter(
                    option__game_id=game_id
                )
...
class StartingPointSystemAdmin(admin.ModelAdmin):
    ...
    list_filter = (
        "code",
    )  # Allows filtering by the 'code' field on the right side of the list page
...
class ResultConfigAdmin(admin.ModelAdmin):
    list_display = (
        "game",
        "is_asymmetric",
        "has_starting_player_order",
        "starting_points_system",
        "has_points",
    )
    list_filter = ("game",)
...
class SelectedGameAdmin(admin.ModelAdmin):
    ...
    def rebuild_selected_game_standings(self, request, queryset):
        ...
        self.message_user(
            request,
            f"Standings rebuilt for {queryset.count()} games.",
            messages.SUCCESS,
        )
        ...
        if db_field.name == "selected_game":
            ...
            if obj_id:
                ...
                if ban_decision and ban_decision.league:
                    kwargs["queryset"] = SelectedGame.objects.filter(
