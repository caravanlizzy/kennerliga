...
class LeagueViewSet(ModelViewSet):
    ...
    def standings(self, request, pk=None):
        ...
        return Response(
            LeagueStandingSerializer(qs, many=True).data, status=status.HTTP_200_OK
        )
    ...
    def game_standings(self, request, pk=None):
        ...
        if not sg_id:
            return Response(
                {"detail": "selected_game is required."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        qs = (
            GameStanding.objects.filter(league=pk, selected_game_id=sg_id)
            .select_related("player_profile__user")
            .order_by("rank", "player_profile__profile_name")
        )
        return Response(
            GameStandingSerializer(qs, many=True).data, status=status.HTTP_200_OK
        )
    ...
    def rebuild_standings(self, request, pk=None):
        ...
        selected_games = SelectedGame.objects.filter(league=league).select_related(
            "game"
        )
    ...
    def resolve_tie(self, request, pk=None):
        ...
        if (
            group_key is None
            or reason_raw is None
            or not isinstance(player_order, list)
            or len(player_order) == 0
        ):
            return Response(
                {
        ...
        valid_ids = set(
            LeagueStanding.objects.filter(
                league=league, player_profile_id__in=player_order
        ...
        id_to_priority = {
            pp_id: (max_priority - idx) for idx, pp_id in enumerate(player_order)
        }
...
class LeagueDetailViewSet(ReadOnlyModelViewSet):
    ...
    def retrieve(self, request, *args, **kwargs):
        # The participant prefetch is centralized in
        # ``season.participant_prefetch.prefetched_members`` and is invoked
        # from ``LeagueDetailSerializer.get_members``. Keep this view thin.
        league = self.get_object()
        serializer = LeagueDetailSerializer(league, context={"league": league})
