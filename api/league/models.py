...
class League(models.Model):
    ...
    members = models.ManyToManyField(
        "season.SeasonParticipant", related_name="leagues_member"
    )
    active_player = models.ForeignKey(
        "season.SeasonParticipant", on_delete=models.CASCADE, null=True, blank=True
    )
    status = models.CharField(
        max_length=20, choices=LeagueStatus.choices, default=LeagueStatus.PICKING
    )
...
class LeagueTieResolution(models.Model):
    ...
    league = models.ForeignKey(
        League, on_delete=models.CASCADE, related_name="tie_resolutions"
    )
    ...
    def __str__(self):
        return (
            f"{self.league} {self.group_key} tie resolved: {self.get_reason_display()}"
        )
...
class LeagueTieResolutionEntry(models.Model):
    ...
    resolution = models.ForeignKey(
        LeagueTieResolution, on_delete=models.CASCADE, related_name="entries"
    )
    player_profile = models.ForeignKey(PlayerProfile, on_delete=models.CASCADE)
...
class GameStanding(models.Model):
    ...
    selected_game = models.ForeignKey(
        SelectedGame, on_delete=models.CASCADE, related_name="standings"
    )
    player_profile = models.ForeignKey(PlayerProfile, on_delete=models.CASCADE)
    ...
    league_points = models.DecimalField(
        max_digits=10, decimal_places=2, default=0
    )  # tie-shared per rules
    win_share = models.DecimalField(
        max_digits=10, decimal_places=2, default=0
    )  # 1, fractional, or 0
    decisive_tie_breaker = models.ForeignKey(
        "game.TieBreaker", on_delete=models.SET_NULL, null=True, blank=True
    )
