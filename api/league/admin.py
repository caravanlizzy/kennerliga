...
class LeagueAdmin(admin.ModelAdmin):
    ...
    def rebuild_all_standings(self, request, queryset):
        ...
        self.message_user(
            request,
            f"Standings rebuilt for {queryset.count()} leagues.",
            messages.SUCCESS,
        )
    ...
    def _render_group_form(self, request, league, group_key: str):
        members = (
            LeagueStanding.objects.filter(league=league, unresolved_tie_group=group_key)
        ...
        rows = [
            f"<li>{ls.player_profile.profile_name} (id={ls.player_profile_id})</li>"
            for ls in members
        ]
    ...
    def resolve_ties_view(self, request, league_id):
        ...
        try:
            player_order = [
                int(x.strip()) for x in player_order_raw.split(",") if x.strip()
            ]
        except ValueError:
            return HttpResponseBadRequest(
                "Invalid player_order format; expected comma-separated integers."
            )
        ...
        if set(player_order) != valid_ids:
            return HttpResponseBadRequest(
                "player_order must contain only PlayerProfile IDs present in this league standings."
            )
        ...
        if mismatched:
            return HttpResponseBadRequest(
                "Provided group_key does not match current standings groups."
            )
        ...
        LeagueTieResolutionEntry.objects.bulk_create(
            [
                LeagueTieResolutionEntry(
                    resolution=resolution, player_profile_id=pp_id, order_index=idx + 1
                )
        ...
        LeagueStanding.objects.bulk_update(
            standings, ["tie_break_priority", "unresolved_tie_group"]
        )
...
class LeagueStandingAdmin(admin.ModelAdmin):
    ...
    def rebuild_view(self, request, ls_id):
        ...
        self.message_user(
            request, "Overall standings rebuilt successfully.", messages.SUCCESS
        )
    ...
    def rebuild_league_standings(self, request, queryset):
        ...
        self.message_user(
            request,
            f"Overall standings rebuilt for {len(leagues)} leagues.",
            messages.SUCCESS,
        )
...
class GameStandingAdmin(admin.ModelAdmin):
    ...
    def rebuild_game_standings(self, request, queryset):
        ...
        self.message_user(
            request,
            f"Standings rebuilt for {len(selected_games)} games.",
            messages.SUCCESS,
        )
